...
export const Pokemons = () => {
  /*  Our client asked us to build application which will list all Pokemons sorted by name.
      Unfortunately pokeApi has no sorting functions so we need to fetch full list of 1118 Pokemons' species
      and sort it on our end.
      For the same reason we can't use APIs built in pagination system, because it is achieved by offsetting and
      limiting the list of Pokemons listed by ID not by name. It is not a big set of data
      because list endpoint sends only the name of the Pokemon and URL to the endpoint from where we can fetch all details.
